...
const PLAIN_SAMPLE =
	'' +
	`<MyLabelInputPair>
  <MyLabel for="text-id">Date:</MyLabel>
  <MyDateInput />
</MyLabelInputPair>`;
...
export default function DateInputPage() {
	const form = useMyForm<FormType>('date_properties');
	...
	useEffect(() => {
		...
		if (!propData.validator && combinedProps.length < 60)
			sample = sample.replace('<MyDateInput />', '<MyDateInput ' + combinedProps + ' />');
		else sample = sample.replace('<MyDateInput />', '<MyDateInput\n    ' + props.join('\n    ') + '\n  />');
	...
	return (
		<div className="wrapper">
			<div className="documentations">
				<section>
					<h1>My Date Input</h1>
					<p>A simple date input, does not handle timezone</p>
				</section>
				...
				<section className="property-section">
					<MyForm formId={form.formId}>
						...
						<ul>
							...
							<ul>
								<li>
									<MyDateInput id="defaultValue" name="defaultValue" />
								</li>
							</ul>
