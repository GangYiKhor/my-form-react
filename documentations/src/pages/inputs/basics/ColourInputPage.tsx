...
const validatorFn = {
	notWhiteReject: function notWhiteRejectFn(data: string) {
		if (data !== '#ffffff') return true;
		return false;
	},
	notWhiteReason: function notWhiteReasonFn(data: string) {
		if (data === '#ffffff') return 'Colour must not be white!';
		return true;
	},
};
...
type ExampleFormType = { [key: string]: MyColourType };

export default function ColourInputPage() {
	const form = useMyForm<FormType>('colour_properties');
	...
	useEffect(() => {
		...
		if (!propData.validator && combinedProps.length < 60)
			sample = sample.replace('<MyColourInput />', '<MyColourInput ' + combinedProps + ' />');
		else sample = sample.replace('<MyColourInput />', '<MyColourInput\n    ' + props.join('\n    ') + '\n  />');
				...
				<section className="property-section">
					<MyForm formId={form.formId}>
						...
						<ul>
							...
							<ul>
								<li>
									<MyColourInput id="defaultValue" name="defaultValue" />
								</li>
							</ul>
							...
							<ul>
								<li>
									<MySelectInput<string | number>
										...
										placeholder="None (Default)"
										options={[
											{ label: 'Not white (Reject only)', value: 'notWhiteReject' },
											{ label: 'Not white (With reason)', value: 'notWhiteReason' },
										]}
									/>
								</li>
							</ul>
