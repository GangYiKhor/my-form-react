...
const PLAIN_SAMPLE =
	'' +
	`<MyLabelInputPair>
  <MyLabel for="text-id">Password:</MyLabel>
  <MyPasswordInput />
</MyLabelInputPair>`;
...
export default function PasswordPage() {
	const form = useMyForm<FormType>('password_properties');
	...
	useEffect(() => {
		...
		if (!propData.validator && combinedProps.length < 60)
			sample = sample.replace('<MyPasswordInput />', '<MyPasswordInput ' + combinedProps + ' />');
		else sample = sample.replace('<MyPasswordInput />', '<MyPasswordInput\n    ' + props.join('\n    ') + '\n  />');
