...
const PLAIN_SAMPLE =
  ...
  <MyLabel for="text-id">Checkbox:</MyLabel>
  <MyCheckboxInput />
</MyLabelInputPair>`;
...
type FormType = {
	...
	prefix: string;
	suffix: string;
	uncheckValue: { str: string; number: string };
	checkedValue: { str: string; number: string };
...
export default function CheckboxInputPage() {
	const form = useMyForm<FormType>('checkbox_properties');
	...
	useEffect(() => {
		...
		if (propData.uncheckValue) props.push(`uncheckValue={${JSON.stringify(propData.uncheckValue)}}`);
		if (propData.checkedValue) props.push(`checkedValue={${JSON.stringify(propData.checkedValue)}}`);
		...
		if (combinedProps.length < 60)
			sample = sample.replace('<MyCheckboxInput />', '<MyCheckboxInput ' + combinedProps + ' />');
		else sample = sample.replace('<MyCheckboxInput />', '<MyCheckboxInput\n    ' + props.join('\n    ') + '\n  />');
					...
					<MyForm<ExampleFormType> formId="example" onSubmit={(_, data) => setData(data)}>
						<MyLabelInputPair arrangement="horizontal">
							...
							<MyCheckboxInput
								...
								uncheckValue={propData.uncheckValue}
								checkedValue={propData.checkedValue}
				...
				<section className="property-section">
					<MyForm formId={form.formId}>
						...
						<ul>
							...
							<li>
								<code className="property-name">checkedValue</code>: Internal value when input is checked (Will be set
								to checked if manually updated to the same value using deepEqual)
							</li>
							<ul>
								<li>
									<MySelectInput
							...
							<li>
								<code className="property-name">uncheckValue</code>: Internal value when input is unchecked
							</li>
							<ul>
								<li>
									<MySelectInput
										id="uncheckValue"
										name="uncheckValue"
										placeholder="undefined (Default)"
										options={[
											{ label: 'Four', value: { str: 'FOUR', number: 4 } },
											{ label: 'Five', value: { str: 'FIVE', number: 5 } },
											{ label: 'Six', value: { str: 'SIX', number: 6 } },
										]}
									/>
								</li>
							</ul>
