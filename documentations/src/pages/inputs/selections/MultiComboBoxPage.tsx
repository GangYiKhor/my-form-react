...
const PLAIN_SAMPLE =
	'' +
	`<MyLabelInputPair>
  <MyLabel for="text-id">Multi Combo Box:</MyLabel>
  <MyMultiComboBoxInput />
</MyLabelInputPair>`;
...
export default function MultiComboBoxPage() {
	const form = useMyForm<FormType>('multicombobox_properties');
	...
	useEffect(() => {
		...
		if (propData.selectedHeight) props.push(`selectedHeight={${propData.selectedHeight}}`);
		if (propData.filterOnType) props.push('filterOnType');
		if (propData.selectedNoWrap) props.push('selectedNoWrap');
		...
		if (propData.removeIfManualUpdateIsInvalid) props.push('removeIfManualUpdateIsInvalid');
		const combinedProps = props.join(' ');
		if (combinedProps.length < 60)
			sample = sample.replace('<MyMultiComboBoxInput />', '<MyMultiComboBoxInput ' + combinedProps + ' />');
		else
	...
	return (
		<div className="wrapper">
			<div className="documentations">
				<section>
					<h1>My Multi Combo Box Input</h1>
					<p>A customisable combo box input with multi selections</p>
				</section>
					...
					<MyForm<ExampleFormType> formId="example" onSubmit={(_, data) => setData(data)}>
						<MyLabelInputPair>
							...
							<MyMultiComboBoxInput
								...
								selectedHeight={propData.selectedHeight}
								selectedNoWrap={propData.selectedNoWrap}
								...
								removeIfManualUpdateIsInvalid={propData.removeIfManualUpdateIsInvalid}
							/>
						</MyLabelInputPair>
				...
				<section className="property-section">
					<MyForm formId={form.formId}>
						...
						<ul>
							...
							<li>
								<code className="property-name">selectedHeight</code>: Max height of the selected list, default
								unlimited
							</li>
							<ul>
								<li>
									<MyPositiveNumberInput id="selectedHeight" name="selectedHeight" />
								</li>
							</ul>

							<li>
								<code className="property-name">selectedNoWrap</code>: Set nowrap to selected item
							</li>
							<ul>
								<li>
									<MySelectInput
							...
							<li>
								<code className="property-name">removeIfManualUpdateIsInvalid</code>: Remove the value when manually
								updated value is not found in the options
							</li>
							<ul>
								<li>
									<MySelectInput
										id="removeIfManualUpdateIsInvalid"
										name="removeIfManualUpdateIsInvalid"
										placeholder="Enabled (Default)"
										options={[{ label: 'Disabled', value: true }]}
									/>
								</li>
